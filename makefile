...
install:
	go generate
	go build -o $(GOPATH)/bin/$(BINARY_NAME) main.go metadata.go migration.go model.go service.go sample.go resources.go

custom-install:
	go generate
	go build -o $(INSTALL_PATH)/$(BINARY_NAME) main.go metadata.go migration.go model.go service.go sample.go resources.go
